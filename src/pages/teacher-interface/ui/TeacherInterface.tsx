...
export const TeacherInterface: FC = () => {
    ...
    const [choise, setChoise] = useState<string>(userRole === UserRole.ROP ? "coverPage" : "selectTemplate")
    const jsonData = useStore.getState().jsonData
    const navigate = useNavigate()

    if (!Object.keys(jsonData).length) {
        if (userRole === UserRole.ROP) navigate("/manager")
        if (userRole === UserRole.TEACHER && choise !== "selectTemplate") setChoise("selectTemplate")
    }
